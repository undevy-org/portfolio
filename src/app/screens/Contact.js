...
import { Mail, Globe, ExternalLink, Copy, MessageCircle, Linkedin } from 'lucide-react';
    ...
    <ScreenWrapper>
      <div className="flex flex-col gap-3 mb-4">
        ...
        {contactData.linkedin && (
          <button
            onClick={() => handleExternalLink('LinkedIn', contactData.linkedin)}
            className="w-full p-3 border rounded flex items-center justify-between transition-colors border-primary bg-hover"
          >
            <div className="flex items-center">
              <Linkedin className="w-5 h-5 mr-3 text-command" />
              <span className="text-white-black">
                {contactData.linkedin.replace(/^https?:\/\/(www\.)?/, '')}
              </span>
            </div>
